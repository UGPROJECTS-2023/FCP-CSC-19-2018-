...
if(isset($_GET['rowId'])){
	$rowInfo=$_GET['rowId'];
	$result="UPDATE mainclearance SET CoordinatorStatus ='Accepted', Coordinator_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
}

if(isset($_GET['rowId2'])){
	$rowInfo=$_GET['rowId2'];
	$result="UPDATE mainclearance SET CoordinatorStatus ='Declined', Coordinator_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
...
<?php
$sql=mysqli_query($db,"SELECT *from mainclearance where  CoordinatorStatus ='Declined';");
$count=1;
