...
if(isset($_GET['rowId'])){
	$rowInfo=$_GET['rowId'];
	$result="UPDATE mainclearance SET DoSStatus ='Accepted', DoS_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
}

if(isset($_GET['rowId2'])){
	$rowInfo=$_GET['rowId2'];
	$result="UPDATE mainclearance SET DoSStatus ='Declined', DoS_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
...
<?php
$sql =mysqli_query($db,"SELECT * FROM mainclearance WHERE PhysicsLabStatus = 'No respond yet'");
$count=1;
