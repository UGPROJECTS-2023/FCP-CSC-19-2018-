...
if(isset($_GET['rowId'])){
	$rowInfo=$_GET['rowId'];
	$result="UPDATE mainclearance SET ComputerLabStatus ='Accepted', CompLab_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
}

if(isset($_GET['rowId2'])){
	$rowInfo=$_GET['rowId2'];
	$result="UPDATE mainclearance SET ComputerLabStatus ='Declined', CompLab_Cleared_By='$fullname' WHERE RegNo='$rowInfo'";
	$sql=mysqli_query($db,$result);
...
<?php
$sql=mysqli_query($db,"SELECT *from mainclearance WHERE ComputerLabStatus ='Declined';");
$count=1;
