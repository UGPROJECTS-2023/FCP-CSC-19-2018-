...
if($result>0){
 ...
 if (isset($_POST['apply'])) {
    ...
    $gender=$_POST['gender'];
    $email=$_POST['email'];
    $sql=mysqli_query($db,"INSERT INTO applicants(UserName,StudentName,clinicnumber,RegNo,Course,Program,Faculty,Department,Gender,Email)
											VALUES('$useridentity','$stdname','$clinicno','$regno','$course','$program','$faculty','$department','$gender','$email')");
	 
	$sql=mysqli_query($db,"INSERT INTO mainclearance(StudentName, UserName, RegNo,Faculty,Department,Course)
												VALUES('$stdname', '$useridentity', '$regno','$faculty','$department','$course')");
	
	 /*if ($sql) {
		echo "<script>alert('You've Applied Successfully')</script>;";
		die ("Ok");
	 }
	 else{
		echo "<script>alert('Something went wrong  Retry to apply Again!')</script>;";
	 } */
} 
...
<!-- Page Content -->
    <div id="page-wrapper">
		...
		<div class="jumbotron">
			<h2>You have successfully applied for your Departmental clearance.<center> Thank you.</center></h2>

		</div>
		<div class="row">
			<div class="col-sm-4" ><img src="images/buk.jpg" width="300" height="330" alt="pic">
				<div>
					<p align="justify">
